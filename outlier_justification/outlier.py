...
# This function takes a lot of run time, we need to find a faster library to implement gauss2dfit
...
def psfwidth(image_data,xo,yo,edg):
	...
	for i in range(64):
		...
		for j in range(edg):
			for k in range(edg):
				f=stx[i][j][k]
				numerx+=f*(j-xo[i])*(j-xo[i])
				numery+=f*(k-yo[i])*(k-yo[i])
...
def plotcurve(xax,f,b,X,Y,GX,GY,wx,wy,npp,direc,ct):
	...
	plt.savefig('test3/4.5_'+str(ct)+'_'+direc+'.png',bbox_inches='tight',dpi=200)
...
##outerpath = 'C:/Users/Lisa/Dropbox/CoRoT-2b/Spitzer'
##dirs_all = dirs=os.listdir(outerpath)
##dirs = [k for k in dirs_all if 'r579' in k]
...
for direc in dirs :
	...
	##path = 'C:/Users/Lisa/Dropbox/CoRoT-2b/Spitzer/'+direc+'/ch2/bcd'
	print (path)
	#xn=1
	ct=0
