# CONVOLVING the image data. Subtracting the fluxlist of the smoothed data from the raw flux list
...
def highpassflist(allstackd,op):
	...
	temp=np.apply_along_axis(lambda m: convolve(m, Box1DKernel(50),boundary='extend'), axis=0, arr=allstackd)
	flist2=getflist(temp,op) #smoothed flist
...
def plot3panels(xax,p1,p2,p3,p4,toss,ct):
	...
	plt.savefig(str(ct)+'_4_lightcurve_tim3.png',bbox_inches='tight',dpi=200)
...
def masterfunc1(op):
	...
	panel1,panel2,panel3=highpassflist(allstackd1,op)
	...
	beforenan= np.argwhere(np.isnan(panel3))
	beforenan=beforenan.flatten()
	panel4= sigma_clip(panel3,sigma=5,cenfunc=np.nanmedian)
	tosstemp= [i for i, x in enumerate(panel4.mask) if x]
	panel4=np.asarray(panel4)
	for x in tosstemp:
		panel4[x]=np.nan
	afternan = np.ma.asarray(panel4)
...
#plot3panels(time[25:-25],panels[0][25:-25],panels[1][25:-25],panels[2][25:-25],ct)
