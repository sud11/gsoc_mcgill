...
#Created: 2016-06-11 03:34 PM EST
#Last Modified: 
#Title: High pass filter the lightcurve
#--------------------------------------------------------------
...
def sigma_clipping(image_data):#,fname):
	...
	for i in range (len(image_data)/20):
		...
		if(len(to)>0):
			...
			toss.extend(to)
		fin=i
	# changed allstackd1 to image_data in the below two lines
...
def plot3panels(xax,p1,p2,p3,p4,toss,ct):
	...
	plt.savefig('3.6test/'+'r46482688'+'_'+str(ct)+'.png',bbox_inches='tight',dpi=500)
...
def plotcurve(xax,f1,f2,ct):
	...
	plt.savefig('test3/'+str(ct)+'updchanges.png',bbox_inches='tight',dpi=500)
...
panels = map (lambda t:map(abs,t), panels)
panels=map(normstar,panels)
print "HERE"

panels = map( lambda t: t-1, panels)


panels=map(lambda t: np.sqrt(np.nanmean(np.square(t))), panels)
