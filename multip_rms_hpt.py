...
def plotcurve(xax,flist,ct):
	...
	axes[0].plot(xax,flist[0],'D-',c='k',mec='b',fillstyle='none')
	...
	axes[1].plot(xax,flist[1],'D-',c='k',mec='b',fillstyle='none')
	...
	axes[2].plot(xax,flist[2],'D-',c='k',mec='b',fillstyle='none')
	axes[2].set_ylabel(r'$\sigma$ $clipped$ $RMS$',fontsize=13)
	axes[2].set_xlabel(r'$aperture$ $(pixels)$',fontsize=13)
...
flist=pool1.map(getflist,[allstackd1,allstackd2])
flist3=tossoutframe(allstackd2)
#flist1= getflist(allstackd1)
print 'time for flist', time()-t
t=time()
#flist2=getflist(allstackd2)
#print 'time for flist2', time()-t
xax=np.arange(1.5,4.75,step=0.25)
flist.append(flist3)
